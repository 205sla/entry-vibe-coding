...
## `reach_something` 기반 플랫포머 발판 충돌 패턴

### 문제
"발판 위에 착지" 처리. Entry는 side-from-which-hit 정보 없이 `reach_something(block)` 이 `true`/`false`만 반환.

### 해결 패턴
매 프레임 순서로:
1. 중력 적용 (`move_y(vy); vy -= 0.6`)
2. 각 발판마다 `if reach(발판X) AND vy ≤ 0 AND 아직_안착지:` → **발판 상단 y로 snap, vy=0, on_ground=1, landed=1**
3. 아무 발판도 안 닿았으면 바닥 스냅(y=-40 등) 또는 공중 상태 유지

핵심 아이디어:
- **`vy ≤ 0` 가드**: 위로 점프 중이면 snap 안 함 → 발판 아래에서 부딪혀도 관통해 올라감
- **`landed` per-tick flag**: 여러 발판이 겹칠 때 우선순위(보통 위 발판부터) 하나만 처리
- **landing_y 상수**: 각 발판마다 미리 계산해 하드코딩. 식: `block.entity.y + block_half_height + player_half_height`
- `boolean_and_or` 는 **단락 평가 없음** (→ [별도 항목](#boolean_and_or에-단락-평가-short-circuit-없음--리스트-인덱스-범위-초과-크래시))이지만 `reach_something`·`vy`·`landed` 읽기는 부작용 없어 안전

### 치수 공식

이미지(원본)를 scale로 축소한 스프라이트:
- 블럭: `block.entity.y` = 중심. 상단 = `center_y + (image_height × scale / 2)`
- 플레이어: `player.entity.y` = 중심. 바닥 = `center_y - (image_height × scale / 2)`
- 착지 조건: `player_bottom == block_top`
  → `player_landing_y = block.entity.y + (block_h × block_scale + player_h × player_scale) / 2`

예: 블럭 100×100, scale 0.5 (half=25); 플레이어 200×240, scale 0.4 (half=48).
    `player_landing_y = block.entity.y + 25 + 48 = block.entity.y + 73`

### 시차 스크롤과 결합

플레이어 x 고정 사이드스크롤러에선 `reach_something`이 **스크린 좌표 기준** 체크하므로
월드 offset으로 발판이 스크롤되어 플레이어 아래로 올 때 자동으로 충돌 감지가 작동한다.
발판 아래 통과 → offset 계속 이동 → 발판이 플레이어 밖으로 → `reach=false`로 되며
gravity 가 작동해 다음 발판 또는 바닥으로 낙하.
