// es-hangul 함수 라이브러리 빌드 — spec.mjs → es-hangul_NNN.ent (자동 넘버링).
//
// 출력은 덮어쓰지 않고 _001, _002 … 로 새 파일 (knowledge/feedback 넘버링 규칙).
//
//   node games/es-hangul/build.mjs
...
const OUT = nextOutputPath(__dirname, 'es-hangul');
const r = await writeEnt(spec, OUT);

console.log(`[es-hangul] wrote ${r.outPath} — ${r.size} bytes, ${r.objectCount} objects`);
